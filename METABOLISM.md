...
├── KnowledgeBookComponent.cs   — книги знаний (чтение через интеракцию)
├── KnowledgeMemoryComponent.cs — память героя о веществах и рецептах
└── SubstanceDebugSystem.cs     — быстрый вывод состава по P и памяти по O
```
```
...
- `O`:
  - вывести справочник памяти о веществах/рецептах в консоль
...
Новые компоненты:
- `knowledgeBook` — книга
- `knowledgeMemory` — память игрока

Книгу можно держать в руке и читать через `E`.

После чтения:
- герой запоминает вещества
- герой запоминает известные рецепты
- информация уходит в память (`KnowledgeMemoryComponent`)
- память можно быстро смотреть по `O`
  ...
  "substances": [
    {
      "id": "taurine",
      "name": "Таурин",
      "preparation": "Часто встречается в тонизирующих напитках и вытяжках.",
      "smell": "сладковато и резко"
    }
  ],
  "recipes": [
    {
...
- более серьёзная энциклопедия памяти по категориям
